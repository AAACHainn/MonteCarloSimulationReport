...
import {
  inspectMarketCsvHeaders, parseMarketCsvRow, type CsvRow, type MarketCsvFormat, type MarketCsvIssue,
...
export async function processImportJob(jobId: string) {
  ...
  await prisma.marketDatasetImport.update({
    where: { id: jobId },
    data: { status: "PROCESSING", processedRows: 0, importedBars: 0, totalErrors: 0, errors: "[]", expandedBytes: 0 },
  });
  ...
  const csvState: { format: MarketCsvFormat } = { format: "STANDARD" };
  ...
  try {
    ...
    const parser = decoded.pipe(parse({
      ...
      columns(headers: string[]) {
        const inspected = inspectMarketCsvHeaders(headers);
        csvState.format = inspected.format;
        ...
        return inspected.normalizedHeaders;
      },
    }));
    ...
    const flush = async () => {
      ...
      await prisma.marketDatasetImport.update({
        where: { id: jobId },
        data: { processedRows: rowCount, importedBars: importedBarCount, expandedBytes },
      });
    };
    ...
    for await (const record of parser as AsyncIterable<CsvRow>) {
      ...
      const parsed = parseMarketCsvRow({
        row: record, rowNumber, sequence: importedBarCount, timezone: metadata.timezone, format: csvState.format,
        ...
        }, cmeRootSymbol: metadata.symbol }, previousTime, previousChartSecond,
      });
      rowCount += 1;
      if (parsed.skipped) {
    ...
    if (importedBarCount < 2) {
      ...
      issues.push({
        row: 1,
        reason: csvState.format === "DATABENTO_CME"
    ...
    await prisma.$transaction([
      ...
      prisma.marketDatasetImport.update({
        where: { id: jobId },
        data: { status: "COMPLETED", processedRows: rowCount, importedBars: importedBarCount, expandedBytes, totalErrors: 0, errors: "[]" },
      }),
    ]);
    ...
    await prisma.marketDatasetImport.update({ where: { id: jobId }, data: {
      datasetId: null, status: "FAILED", processedRows: rowCount, importedBars: importedBarCount, expandedBytes,
