...
[docs/ui-guidelines.md](docs/ui-guidelines.md) 是本项目唯一的 UI 设计与实现规范。

- 凡新增页面、组件或修改任何用户可见界面，开始开发前必须完整阅读该规范，交付前完成其中对应的检查清单。
- 优先复用 `components/ui/` 和既有页面模式；可复用的交互与样式应沉淀到基础组件，不得在业务组件中复制实现。
- 局部业务需求可以形成例外，但必须在交付说明中写明原因、影响范围和验证方式。相同例外重复出现时，应抽象为基础组件或同步更新规范。
- UI 修改仍须遵守本文其余架构、国际化、格式化、测试和数据兼容约束。
